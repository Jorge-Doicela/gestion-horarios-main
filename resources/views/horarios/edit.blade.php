    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-8">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-8 animate-fade-in-up">
                ...
                <p class="text-gray-600">Modifique la información del horario: <span class="font-semibold text-cyan-600">{{ $horario->materia->nombre }} - {{ $horario->paralelo->nombre }}</span></p>
            </div>

            <!-- Status Warning -->
            @if ($horario->estado !== 'activo')
                <div class="bg-yellow-50 border border-yellow-200 text-yellow-800 px-6 py-4 rounded-xl mb-6 animate-fade-in-up" style="animation-delay: 0.1s;">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-sm">Este horario está <strong>{{ $horario->estado }}</strong> y no puede ser modificado.</p>
                        </div>
                    </div>
                </div>
            @endif
            ...
            <!-- Error Messages -->
            @if ($errors->any())
                <div class="bg-red-50 border border-red-200 text-red-700 px-6 py-4 rounded-xl mb-6 animate-fade-in-up" style="animation-delay: 0.2s;">
                ...
                <form action="{{ route('horarios.update', $horario->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="paralelo_id" required {{ $disabled }}
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="fecha_inicio" value="{{ $horario->fecha_inicio }}" required {{ $disabled }}
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}">
                            @error('fecha_inicio')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="date" name="fecha_fin" value="{{ $horario->fecha_fin }}" required {{ $disabled }}
                                   class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}">
                            @error('fecha_fin')
                                <span class="text-red-600 text-sm">{{ $message }}</span>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="modalidad" required {{ $disabled }}
                                    class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}">
                                <option value="presencial" {{ $horario->modalidad == 'presencial' ? 'selected' : '' }}>Presencial</option>
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea name="observaciones" rows="4" {{ $disabled }}
                                  class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-cyan-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm resize-none {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}"
                    ...
                    <div class="bg-cyan-50/50 border border-cyan-200 rounded-xl p-4">
                        <div class="flex items-center mb-3">
                            ...
                            <h3 class="text-sm font-semibold text-cyan-800">Información Actual del Horario</h3>
                        </div>
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-4 text-sm">
                            <div class="bg-white/70 px-3 py-2 rounded-lg">
                                <span class="font-medium text-cyan-700">Materia:</span>
                                <div class="text-gray-900 font-semibold">{{ $horario->materia->nombre }}</div>
                            </div>
                            <div class="bg-white/70 px-3 py-2 rounded-lg">
                                <span class="font-medium text-cyan-700">Paralelo:</span>
                                <div class="text-gray-900 font-semibold">{{ $horario->paralelo->nombre }}</div>
                            </div>
                            <div class="bg-white/70 px-3 py-2 rounded-lg">
                                <span class="font-medium text-cyan-700">Docente:</span>
                                <div class="text-gray-900 font-semibold">{{ $horario->docente->nombre }}</div>
                            </div>
                            <div class="bg-white/70 px-3 py-2 rounded-lg">
                                <span class="font-medium text-cyan-700">Estado:</span>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 pt-6">
                        <button type="submit" {{ $disabled }}
                                class="flex-1 bg-gradient-to-r from-cyan-600 to-blue-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-cyan-700 hover:to-blue-700 transition-all duration-200 transform hover:scale-105 shadow-lg {{ $disabled ? 'opacity-50 cursor-not-allowed' : '' }}">
