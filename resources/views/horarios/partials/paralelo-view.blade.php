@php
    $horariosPorParalelo = $horarios->groupBy('paralelo_id');
@endphp

@if ($horarios->count() > 0)
    <div class="p-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach ($horariosPorParalelo as $paraleloId => $horariosParalelo)
                @php
                    $paralelo = $horariosParalelo->first()->paralelo;
                    $totalHorarios = $horariosParalelo->count();
                    $horariosActivos = $horariosParalelo->where('estado', 'activo')->count();
                @endphp
                ...
                <div
                    ...
                    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-bold text-white">{{ $paralelo->nombre }}</h3>
                                <p class="text-blue-100 text-sm">{{ $totalHorarios }} horarios • {{ $horariosActivos }}
                                    activos</p>
                            </div>
                    ...
                    <!-- Contenido del Paralelo -->
                    <div class="p-6">
                        <div class="space-y-4">
                            @foreach ($horariosParalelo->groupBy('dia_id') as $diaId => $horariosDia)
                                @php
