@php
    $horariosPorDia = $horarios->groupBy('dia_id');
...
@if ($horarios->count() > 0)
    <div class="p-6">
        <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            @foreach ($diasOrden as $diaNombre)
                @php
                    $diaHorarios = $horariosPorDia
                        ->filter(function ($horarios, $diaId) use ($diaNombre) {
                            return $horarios->first()->dia->nombre === $diaNombre;
                        })
                        ->first();
                @endphp
                ...
                @if ($diaHorarios)
                    @php
                        $dia = $diaHorarios->first()->dia;
                        $totalHorarios = $diaHorarios->count();
                        $horariosActivos = $diaHorarios->where('estado', 'activo')->count();
                        $horariosPresenciales = $diaHorarios->where('modalidad', 'presencial')->count();
                        $horariosVirtuales = $diaHorarios->where('modalidad', 'virtual')->count();
                    @endphp
                    ...
                    <div
                        ...
                        <div class="bg-gradient-to-r from-purple-500 to-pink-600 px-6 py-4">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-purple-100 text-sm">{{ $totalHorarios }} horarios •
                                        {{ $horariosActivos }} activos</p>
                                </div>
                        ...
                        <div class="p-6">
                            <div class="space-y-4">
                                @foreach ($diaHorarios->groupBy('hora_id') as $horaId => $horariosHora)
                                    @php
                                        $hora = $horariosHora->first()->hora;
                                    @endphp
                                    ...
                                    <div class="border-l-4 border-purple-200 pl-4">
                                        ...
                                        <div class="space-y-3">
                                            @foreach ($horariosHora->sortBy('paralelo.nombre') as $horario)
                                                @php
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex items-center justify-between mb-2">
                                                        ...
                                                        <span
                                                            class="text-sm text-gray-600">{{ $horario->paralelo->nombre }}</span>
                                                    </div>
