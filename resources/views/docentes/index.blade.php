    ...
    <div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="text-center mb-8 animate-fade-in-up">
                ...
                <h1 class="text-3xl font-bold text-gray-900 mb-2">Gestión de Docentes</h1>
                <p class="text-gray-600">Administre el personal docente del sistema de horarios</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 animate-fade-in-up" style="animation-delay: 0.2s;">
                <div class="bg-white/80 backdrop-blur-md rounded-xl px-4 py-2 border border-white/20">
                    <span class="text-sm text-gray-600">Total de docentes:</span>
                    <span class="font-semibold text-indigo-600 ml-1">{{ $docentes->total() }}</span>
                </div>
                <div class="bg-white/80 backdrop-blur-md rounded-xl px-4 py-2 border border-white/20">
                    <span class="text-sm text-gray-600">Activos:</span>
                    <span class="font-semibold text-indigo-600 ml-1">{{ $docentes->count() }}</span>
                </div>
                <div class="bg-white/80 backdrop-blur-md rounded-xl px-4 py-2 border border-white/20">
                    <span class="text-sm text-gray-600">Materias asignadas:</span>
                    <span class="font-semibold text-indigo-600 ml-1">{{ $docentes->sum(function($docente) { return $docente->materias->count(); }) }}</span>
                </div>
            </div>
                ...
                <form method="GET" action="{{ route('docentes.index') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <input type="text" name="search" value="{{ request('search') }}" 
                                   placeholder="Buscar por nombre, email, título, especialidad, materias..."
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="especialidad" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="materia_id" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="sort_by" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm">
                                ...
                                <option value="especialidad" {{ request('sort_by') == 'especialidad' ? 'selected' : '' }}>Especialidad</option>
                        ...
                        <div class="space-y-2">
                            ...
                            <select name="sort_direction" class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition-all duration-200 bg-white/50 backdrop-blur-sm">
                        ...
                        <div class="space-y-2">
                            ...
                            <div class="text-sm text-gray-600 px-4 py-2 bg-gray-50 rounded-xl">
                                Mostrando {{ $docentes->firstItem() ?? 0 }} - {{ $docentes->lastItem() ?? 0 }} de {{ $docentes->total() }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row gap-4 items-center justify-between">
                        ...
                        <div class="text-sm text-gray-600">
                            <span class="font-semibold">{{ $docentes->total() }}</span> docentes encontrados
                        </div>
                    </div>
                </form>
            </div>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl flex items-center justify-center mr-4">
                                                <span class="text-white font-bold text-lg">{{ substr($docente->nombre, 0, 2) }}</span>
                                            </div>
                                            <div>
                                                <div class="text-sm font-semibold text-gray-900">{{ $docente->nombre }}</div>
                                                <div class="text-xs text-gray-500">ID: {{ $docente->id }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div class="w-8 h-8 bg-gradient-to-br from-yellow-400 to-orange-500 rounded-full flex items-center justify-center mr-2">
                                                <span class="text-white font-bold text-xs">{{ $docente->materias->count() }}</span>
                                            </div>
                                            <div class="max-w-xs">
                                                <div class="text-sm text-gray-900 truncate">
                                                    {{ $docente->materias->pluck('nombre')->take(2)->join(', ') }}
                                                    @if($docente->materias->count() > 2)
                                                        <span class="text-gray-500">+{{ $docente->materias->count() - 2 }} más</span>
                                                    @endif
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div class="flex items-center space-x-3">
                                            ...
                                            <form action="{{ route('docentes.destroy', $docente) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" 
                                                        ...
                                                        data-confirm-message="¿Está seguro de eliminar el docente '{{ $docente->nombre }}'? Esta acción no se puede deshacer."
            ...
            @if($docentes->isEmpty())
                <div class="text-center py-12 animate-fade-in-up" style="animation-delay: 0.4s;">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 mb-2">No hay docentes registrados</h3>
                    <p class="text-gray-600 mb-6">Comience registrando el primer docente del sistema.</p>
                    <a href="{{ route('docentes.create') }}" 
                       class="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-6 py-3 rounded-xl font-semibold hover:from-indigo-700 hover:to-purple-700 transition-all duration-200 transform hover:scale-105 shadow-lg">
